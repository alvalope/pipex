...
/*   Updated: 2023/05/25 21:59:33 by alvalope         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_first_command(t_pipex *p, char *command)
{
	/*while (command[i] == 32 || (command[i] > 8 && command[i] < 14))
		i++;*/
	p->args1 = ft_split_quote(command, ' ');
	...
	if (access(p->path1, F_OK) == -1 || access(p->path1, X_OK) == -1)
	{
		...
		if (access(p->path1, F_OK) == -1 || access(p->path1, X_OK) == -1)
		{
			/*if (ft_strncmp(p->args1[0], "exit", 4) == 0)
			{
				free(p->path1);
				p->path1 = ft_strjoin("", p->args1[0]);
			}	
			else
			{*/
...
{
	p->args2 = ft_split_quote(command, ' ');
	...
	if (access(p->path2, F_OK) == -1 || access(p->path2, X_OK) == -1)
	{
		...
		if (access(p->path2, F_OK) == -1 || access(p->path2, X_OK) == -1)
		{
			/*if (ft_strncmp(p->args2[0], "exit", 4) == 0)
			{
				free(p->path2);
				p->path2 = ft_strjoin("", p->args2[0]);
			}	
			else
			{*/
